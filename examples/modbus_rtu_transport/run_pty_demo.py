...
def main():
    parser = argparse.ArgumentParser(description="Run the Modbus RTU diagnostics PTY demo.")
    parser.add_argument("--device", required=True)
    parser.add_argument("--tester", required=True)
    parser.add_argument("--address", default="17")
    ...
    try:
        ...
        tester = subprocess.Popen(
            [str(Path(args.tester)), "--port", tester_port, "--address", args.address],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
        )
        ...
        if not bridge_until_done(device, tester, master_device, master_tester, 5.0):
            device.kill()
            tester.kill()
            print("modbus demo: timed out while bridging PTYs")
            return 1
    ...
    print("+ " + str(Path(args.tester)) + " --port " + tester_port + " --address " + args.address,
          flush=True)
